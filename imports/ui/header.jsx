...
import { SettingsIcon, BackIcon, MenuIcon, LogoutIcon, DeleteIcon, CloseIcon, SearchIcon, LeftArrowIcon, EyeIcon, MenuIcon2 } from  "/imports/other/styles/icons";
import {
  useTracker
...
export default function Header( props ) {
  ...
  const {match, location, history, setSearch, search, toggleRevealPassword} = props;
  ...
  (location.pathname === "/" || location.pathname.includes("list") || location.pathname === listFolders || location.pathname.includes("deleted")) &&
  ...
  return (
    <PageHeader>
      {
      (  folderID || location.pathname.includes("deleted") || location.pathname === editCurrentUser ) &&
      <LinkButton
        font="white"
        onClick={(e) => {
          e.preventDefault();
      ...
      {
        !openSearch &&
      <h1 onClick={(e) => props.history.push(listFolders)}>{title}</h1>
    }
